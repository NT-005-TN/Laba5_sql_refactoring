...
public interface ClientRepository extends JpaRepository<Client, Long> {
    ...
    // ОПТИМИЗИРОВАННО: убраны CASE из ORDER BY
    @Query("""
        ...
        """)
    Page<Client> findClientsByCriteriaSimple(
    ...
    // ОПТИМИЗИРОВАННО: замена подзапросов на JOIN с GROUP BY
    @Query("""
        SELECT c,
               COUNT(CASE WHEN o.status = 'COMPLETED' THEN 1 END) as completedOrders,
               COALESCE(SUM(o.finalAmount), 0) as lifetimeValue,
               MAX(o.orderDatetime) as lastPurchaseDate
        FROM Client c
        LEFT JOIN c.orders o
        WHERE c.isPermanent = true
        GROUP BY c.id, c.firstName, c.lastName, c.patronymic, c.phone, 
                 c.isPermanent, c.createdAt, c.updatedAt, c.user
