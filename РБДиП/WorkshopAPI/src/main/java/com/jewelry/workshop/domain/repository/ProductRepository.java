...
public interface ProductRepository extends JpaRepository<Product, Long> {
    ...
    // ОПТИМИЗИРОВАННО: выборка конкретных полей
    @Query("""
        SELECT p.id, p.name, p.price, p.inStock, p.isAvailable 
        FROM Product p 
        WHERE p.type = :type AND p.isAvailable = true
        """)
    List<Object[]> findProductSummary(@Param("type") String type);
